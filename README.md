...
Добавления опций к запускаемому процессу через внешний файл возможно следующим образом:

На примере cron:

В файле /etc/default/cron прописываются сами значения переменных.
Далее в unit-файле в разделе [Service] указывается, откуда значения параметров забирать и указывается сам параметр через $: 
```
...
![3_7_5](pictures/3_7_5.JPG)
 ...
 +  Редактируем настройки сети (/etc/network/interfaces):
```
...
```
После рестарта, перелогирования в ifconfig появится агрегатный интерфейс bond0.
________________________
...
__1.  Подключитесь к публичному маршрутизатору в интернет. Найдите маршрут к вашему публичному IP.__
```
...
show bgp x.x.x.x/32
```
Проверяем наш публичный IP (В моём случае – этот белый ip – динамический адрес на BRAS оператора):

![3_8_1](pictures/3_8_1.JPG)
 
Подключаемся к публичному маршрутизатору:
 
![3_8_2](pictures/3_8_2.JPG)
 
Выполнение команды show ip route 83.149.44.6:
 
![3_8_3](pictures/3_8_3.JPG)
 
Выполнение команды show bgp 83.149.44.6:
 
![3_8_4](pictures/3_8_4.JPG)
________________________
 
__2.  Создайте dummy0 интерфейс в Ubuntu. Добавьте несколько статических маршрутов. Проверьте таблицу маршрутизации.__
 
Текущая таблица маршрутизации (до прописывания статики):

![3_8_5](pictures/3_8_5.JPG)
 
Необходимо загрузить модуль «dummy», можно также добавить опцию «numdummies=2» чтобы сразу создалось два интерфейса dummyX:
```
...
lsmod | grep dummy
```
![3_8_6](pictures/3_8_6.JPG)
 
Посмотрим создались ли интерфейсы:
```
ifconfig -a | grep dummy
```
![3_8_7](pictures/3_8_7.JPG)
 
Повесим ip-адреса на dummy-интерфейсы:
```
~$ sudo ip addr add 192.168.1.111/24 dev dummy0
~$ sudo ip addr add 192.168.1.112/24 dev dummy1
```
Внесём изменения в файл /etc/network/interfaces:
 
![3_8_8](pictures/3_8_8.JPG)
 
Проверим таблицу маршрутизации:
a)

![3_8_9](pictures/3_8_9.JPG)
 
b)
 
![3_8_10](pictures/3_8_10.JPG)
________________________
...
Проверяем открытые TCP-порты ~$ sudo ss -tnlp:
 
![3_8_11](pictures/3_8_11.JPG)
...
Проверяем используемые UDP-порты ~$ sudo ss -unap:

![3_8_12](pictures/3_8_12.JPG)
 
    53-ий порт для DNS
    68-ой порт для DHCP (Протокол DHCP является клиент-серверным, то есть в его работе участвуют клиент DHCP и сервер DHCP. Передача данных производится при помощи протокола UDP. По умолчанию запросы от клиента делаются к серверу на порт 67, сервер в свою очередь отвечает клиенту на порт 68, выдавая адрес IP и другую необходимую информацию, такую, как сетевую маску, маршрутизатор и серверы DNS).
________________________
...
Есть скрипт:
 ...
 d=$a+$b
 e=$(($a+$b))
 Какие значения переменным c,d,e будут присвоены? Почему?__
